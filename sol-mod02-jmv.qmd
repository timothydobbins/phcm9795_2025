...
```{r fig-act2-1}
#| echo: false
#| fig-cap: Relative frequency of low birth weight by mother's age group

babies <-readRDS("data/activities/Activity_2.1.rds")

babies$"Birth weight" <- babies$LOW
babies$"Smoke" <- babies$SMOKE
babies$"Age group" <- babies$AgeGrp


# Create a two-way table showing row percents
j <- contTables(data=babies, rows="Birth weight", cols="Age group", pcCol=TRUE,
...
@fig-act2-1 was also created using the `contTables` function, and is based on thinking of constructing a two-way table consisting of `Age group` and `Low birth weight`, and requesting percents within each age group:

![](img/mod02/jamovi/act2.1.2.png)

We can request a bar-chart in the **Plots** section, where the x-axis of the plot is `Age group` (i.e. the columns; [1]), and we want the bars to be stacked [2]. We request the y-axis to be percentages [3], and the percentages to be the column percents [4]:

![](img/mod02/jamovi/act2.1.3.png)

The final figure is shown below. Note that the x-axis on this figure is not ideal. jamovi currently does not allow editing of figures, so we are stuck with this version.

![Relative frequency of low birth weight by mother's age group](img/mod02/jamovi/act2.1.4.png)
...
b) We are interested in determining the probability that 60 or more participants prefer the new drug, so we define `x` as 59.
