...
t_cross <- tbl_cross(pbc, sex, stage, missing="no",
...
t_rwo <- tbl_cross(pbc, sex, stage, missing="no",
                     percent = "row",
                  label = list(sex ="Sex", stage="Stage"),
                  digits = 0)
...
flextable(tab1_pres, cwidth = c(1.5, 5.5))
```
...
Here, we used the `dbinom` function with x=3, size=6, and prob=0.198. This gives an answer of 0.08 (see @sec-binom-r for details).
...
We can use the `pbinom` function with the `lower.tail=FALSE` option (@sec-binom-r).
...
Here we canuse the `pbinom` function.
